...
object MetaData {
    ...
    fun extractNamesFromMetadata(model: MappedByteBuffer): List<String> {
        try {
            ...
            // Remove "Shafawee" from each label, if present
            return list.map { it.replace("Shafawee", "").trim() }
//            return list
        } catch (_: Exception) {
            return emptyList()
        }
...
//                line = reader.readLine()
                // Remove "Shafawee" from the label, if present
                labels.add(line.replace("Shafawee", "").trim())
                line = reader.readLine()
            }
